...
public class Overflow
{
    ...
    */
    private static List<Integer> power(int n, int m)
    {
        ...
        for ( int i = 1; i <= m; i++ )
        {
            ...
            assert nextPower > result.get(result.size() - 1) : "power: Integer overflow";
    ...
    */
    private static List<Float> power_fp(float n, int m)
    {
        ...
        {
            power = power * n;
            if (i > 1) { assert power > result.get(result.size() - 1) : "Floating-point overflow"; }
            result.add(power);
        }
        return result;
    }
    ...
    */
    private static List<Float> geom_fp(float n, int m)
    {
        ...
        for ( int i = 0; i <= m; i++ )
        {
            ...
            if (i > 1) { assert power == result.get(result.size() - 1) : "Floating point underflow"; }
            sum = sum + power;
            result.add(sum);
        }
        return result;
